 ...
 * Contains static classes wifi and ble.
 * Classes wifi and ble are identical. The only difference is androidInterface calls to wifi and ble modules.
 */
const netScanner = {
    wifi: {
    ...
    filter: function (points, options) {
        ...
        console.info("options.names is " + options.names);
        let result = [];
        for (const point of points) {
            ...
            if (options.prefixNames != null) {
                console.info("prefixNames is " + JSON.stringify(options.prefixNames));
            ...
            if (options.ids != null) {
                console.info("ids is " + JSON.stringify(options.ids));
            ...
            if (options.filters != null) {
                console.info("filters is" + JSON.stringify(options.filters));
    ...
    if (data != null) {
        if (data.error != null) {
            ...
            for (const onsuccessAndOpt of onsuccessAndOptList) {
                ...
                console.info("pbOnNetworkScanned callback: "+ JSON.stringify(filteredPoints));
...
function pbOnAvailabilityChanged(networkDataStr) {
    console.info("pbOnAvailabilityChanged is CALLED with networkDataStr="+networkDataStr);
...
function pbOnNetworkScanned(networkDataStr) {
    // console.info("pbOnNetworkScanned is CALLED with networkDataStr="+networkDataStr);
    const networkData = JSON.parse(networkDataStr);
...
function pbOnPageLoaded(networkDataStr) {
    console.info("pbOnPageLoaded is CALLED with networkDataStr"+networkDataStr);
    const networkData = JSON.parse(networkDataStr);
